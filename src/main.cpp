...
extern "C" void app_main(void) {
  ...
  uint8_t buf[] = {0x80, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x02, 0x45,
                   0x6d, 0xff, 0xdd, 0xdc, 0x02, 0x45, 0x6d, 0xff, 0xdd, 0xdc, 0x10, 0xb5};

  while (true) {
    // wifi_mode_t mode;
    // ESP_ERROR_CHECK(esp_wifi_get_mode(&mode));
    // // enum wifi_interface_t if = WIFI_IF_STA;
    ...
    // {
    //   uint8_t buf[1000];
    //   // int len = odid_wifi_build_nan_sync_beacon_frame((char *)mac, buf,
    //   1000); int len = odid_wifi_build_message_pack_nan_action_frame(&data,
    //   (char*)mac, 0, buf, 1000); if (len < 0) {
    //     printf("error: %d\n", len);
    //   }
    //   ESP_ERROR_CHECK(esp_wifi_80211_tx((wifi_interface_t)0, buf, len,
    //   false)); printf("byte: %02x\n", buf[0]);
    // }
    // uint8_t buf[1000];
