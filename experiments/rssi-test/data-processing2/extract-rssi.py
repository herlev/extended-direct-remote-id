...
def group_data(data):
  ...
  for prev, next in sliding_window(data):
    ...
    if next[1] < prev[1] or abs(next[1] - prev[1]) > 10000000:
      groups.append([])
...
receiver_readings = {}
receiver_names = list(char_range('a', 'r'))
receiver_names.remove('e')

for receiver in receiver_names:
  data = np.loadtxt(f"../data-experiment2/Measurements/{receiver}.txt",delimiter="\t",skiprows=7,usecols=(0,1),dtype=int)
