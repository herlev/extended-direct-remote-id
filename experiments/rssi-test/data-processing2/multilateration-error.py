...
def main():
    ...
    db0 = rssi_calibration[4]["Mean"]
    db0_dist = rssi_calibration[4]["Distance"]/100
    ...
    # Calculate UTM coordinates of transmitter locations for error estimation
    ...
    print(pathloss_list)
    for p in tqdm(range(len(pathloss_list))): # loop through a range of pathlosses
        pathloss = pathloss_list[p]
        pos_est_list = []
        all_points_est = []
        for j in range(len(tx_points)): # Loop through all transmitter locations
            print(j)
            if j == 8: # We skip tranmitter location 8 since this is outside the circle.
                continue
            rssi = np.loadtxt(f"points/{j}.txt",delimiter=",",skiprows=0,dtype=int)
            pos_est_list = []
            for i in range(len(rssi)):
                RSSI_list = rssi[i,1:]
                distance_list = distance_estimate(RSSI_list,db0=db0,pathloss=pathloss,reference_distance=db0_dist)
    ...
    plt.xlabel("Pathloss exponent n")
    plt.ylabel("Error [m]")
    plt.grid()
    plt.legend()
    plt.savefig("../figures/multilateration_error_experiment2.png")
    plt.show()
