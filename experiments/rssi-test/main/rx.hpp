...
void log_payload(ByteBuffer buffer, signed rssi) {
  ...
  rssi_table_elem.latest_count = odid_msg.msg_counter;
  rssi_table_elem.timestamp = timestamp;
  rssi_table_elem.rssi = rssi;
  return;
}
