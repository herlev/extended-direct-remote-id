...
#include <stdio.h>
uint32_t counter = 0;
uint8_t bt5_mac[6] = {0xc0, 0xde, 0x52, 0x00, 0x00, 0x04};
...
static void gap_ble_ext_scan_cb(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param) {
  ...
  case ESP_GAP_BLE_EXT_ADV_REPORT_EVT:
    if (!(param->ext_adv_report.params.event_type & ESP_BLE_GAP_SET_EXT_ADV_PROP_LEGACY)) { // not legacy BT
      if (!c_array_equal(&bt5_mac[0], param->ext_adv_report.params.addr, sizeof(bt5_mac))) {
        return;
      }
      ...
      if (c_array_equal(&data[1], UUID, sizeof(UUID)) && data[4] == ODID_appcode) {
        ...
        counter++;
        printf("%lu\t%lld\t%d\t%d\t", counter, esp_timer_get_time(), param->ext_adv_report.params.rssi,param->ext_adv_report.params.adv_data_len);
