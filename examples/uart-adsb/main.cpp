...
void send(uart_port_t uart_num, uint8_t system_id, uint8_t component_id, uint8_t channel, char *callsign, uint32_t icao,
  ...
  int len = mavlink_msg_adsb_vehicle_pack(system_id, component_id, &msg, icao, lat * 10000000,
                                               lon * 10000000, 1, 30000, 0, 0, 0, callsign, 14, 1, 0b11111, 6969);
...
extern "C" void app_main(void) {
  ...
  // Write data to UART.
  while (true) {
    send(uart_num, 3, 156, 5, cs2, 3, 54, 10);
    ESP_ERROR_CHECK(uart_wait_tx_done(uart_num, 100)); // wait timeout is 100 RTOS ticks (TickType_t)
    send(uart_num, 3, 156, 5, cs2, 2, 55, 10);
